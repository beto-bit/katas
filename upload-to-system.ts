...
function parsePayrollSheet(
  ...
  const headers = sheetData[headerRowIndex].map((h: unknown) =>
    h?.toString().trim() || ""
  );
...
async function convertAndUpload(
  ...
  console.log(`\nConversión exitosa:`);
  console.log(`   Nodos: ${uniqueNodes.length}`);
  console.log(`   Aristas: ${uniqueEdges.length}`);

  // Subir a la API
  console.log(`\nSubiendo a la API: ${apiUrl}/api/data/upload`);
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   Nodos guardados: ${result.nodesSaved}`);
      console.log(`   Aristas guardadas: ${result.edgesSaved}`);
    ...
    console.error(`   Asegúrate que el servidor esté corriendo en ${apiUrl}`);
  }
...
if (import.meta.main) {
  ...
  if (args.length < 1) {
    console.log(`
Uso: deno run --allow-read --allow-net --allow-env upload-to-system.ts <archivo_excel> [api_url]

Ejemplos:
  deno run --allow-read --allow-net --allow-env upload-to-system.ts nomina.xlsx
  deno run --allow-read --allow-net --allow-env upload-to-system.ts presupuesto.xlsx http://localhost:8000
`);
    Deno.exit(1);
  }
  ...
  const apiUrl = args[1] || "http://localhost:8000";

  try {
    await convertAndUpload(inputPath, apiUrl);
  } catch (error) {
    console.error("\nError:", error.message);
    Deno.exit(1);
  }
